...
def main():
    ...
    logger.info("Setting up data for discrete model evaluation...")
    ...
    # Build datasets/loaders using discrete dataset
    ...
    logger.info(f"Feature dimensions from discrete dataset:")
    ...
    logger.info("Initializing discrete model for evaluation...")
    ...
    # Load the discrete model checkpoint
    state = torch.load(best_model_path, map_location=device)
    model.load_state_dict(state)
    logger.info(f"Loaded discrete model from {best_model_path}")

    logger.info("Evaluating discrete model on test set...")
    acc, prec, rec, f1 = evaluate_model(model, test_loader, device)
    logger.info(f"DISCRETE MODEL - Test Accuracy: {acc*100:.2f}% | Precision: {prec:.4f} | Recall: {rec:.4f} | F1: {f1:.4f}")

    # Save metrics
    metrics_path = os.path.join(config['output_dir'], 'test_metrics_discrete.json')
    with open(metrics_path, 'w') as f:
        json.dump({"accuracy": acc, "precision": prec, "recall": rec, "f1": f1}, f, indent=2)
    logger.info(f"Saved discrete test metrics to {metrics_path}")
