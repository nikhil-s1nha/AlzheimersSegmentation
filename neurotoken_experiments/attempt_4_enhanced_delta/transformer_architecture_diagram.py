...
def create_attention_heatmap():
    ...
    regions = ['Hippocampus', 'Entorhinal', 'Amygdala', 'Precuneus', 'Posterior Cingulate',
               'Inferior Temporal', 'Middle Temporal', 'Superior Temporal', 'Fusiform', 'Insula']
    
    spatial_attention = np.random.rand(10, 10)
    spatial_attention = (spatial_attention + spatial_attention.T) / 2  # Make symmetric
    np.fill_diagonal(spatial_attention, 1.0)  # Self-attention
    
    im1 = ax1.imshow(spatial_attention, cmap='YlOrRd', aspect='auto')
    ax1.set_xticks(range(len(regions)))
    ax1.set_yticks(range(len(regions)))
    ax1.set_xticklabels(regions, rotation=45, ha='right')
    ax1.set_yticklabels(regions)
    ax1.set_title('Spatial Attention Patterns\n(Brain Region Interactions)', fontsize=14, fontweight='bold')
    
    # Add colorbar
    cbar1 = plt.colorbar(im1, ax=ax1)
    cbar1.set_label('Attention Weight', fontsize=12)
    
    # Temporal attention (visits)
    visits = ['Visit 1', 'Visit 2', 'Visit 3', 'Visit 4', 'Visit 5']
    temporal_attention = np.array([
        [1.0, 0.8, 0.6, 0.4, 0.2],  # Visit 1
        [0.8, 1.0, 0.9, 0.7, 0.5],  # Visit 2
        [0.6, 0.9, 1.0, 0.8, 0.6],  # Visit 3
        [0.4, 0.7, 0.8, 1.0, 0.9],  # Visit 4
        [0.2, 0.5, 0.6, 0.9, 1.0]   # Visit 5
    ])
    
    im2 = ax2.imshow(temporal_attention, cmap='Blues', aspect='auto')
    ax2.set_xticks(range(len(visits)))
    ax2.set_yticks(range(len(visits)))
    ax2.set_xticklabels(visits)
    ax2.set_yticklabels(visits)
    ax2.set_title('Temporal Attention Patterns\n(Visit-to-Visit Dependencies)', fontsize=14, fontweight='bold')
...
def create_attention_visualization():
    ...
    im1 = ax1.imshow(cross_modal, cmap='RdYlBu_r', aspect='auto')
    ...
    ax1.set_title('Cross-Modal Attention\n(Token Type Interactions)', fontsize=12, fontweight='bold')
    
    # Add values
    for i in range(len(modalities)):
        for j in range(len(modalities)):
            ax1.text(j, i, f'{cross_modal[i, j]:.1f}', ha='center', va='center', 
                    color='white' if cross_modal[i, j] < 0.5 else 'black', fontweight='bold')
    
    # 2. Spatial attention (brain regions)
    ...
    im2 = ax2.imshow(spatial_att, cmap='viridis', aspect='auto')
    ax2.set_xticks(range(len(regions)))
    ax2.set_yticks(range(len(regions)))
    ax2.set_xticklabels(regions)
    ax2.set_yticklabels(regions)
    ax2.set_title('Spatial Attention\n(Brain Region Correlations)', fontsize=12, fontweight='bold')
    
    # 3. Temporal attention (visits)
    ...
    im3 = ax3.imshow(temporal_att, cmap='plasma', aspect='auto')
    ax3.set_xticks(range(len(visits)))
    ax3.set_yticks(range(len(visits)))
    ax3.set_xticklabels(visits)
    ax3.set_yticklabels(visits)
    ax3.set_title('Temporal Attention\n(Visit Dependencies)', fontsize=12, fontweight='bold')
    
    # Add values
    for i in range(len(visits)):
        for j in range(len(visits)):
            ax3.text(j, i, f'{temporal_att[i, j]:.1f}', ha='center', va='center', 
                    color='white' if temporal_att[i, j] < 0.5 else 'black', fontweight='bold')
    
    # 4. Attention head analysis
    heads = [f'Head {i+1}' for i in range(8)]
    head_specialization = np.array([
        [0.9, 0.1, 0.2, 0.1, 0.3, 0.2, 0.1, 0.1],  # Spatial
        [0.1, 0.8, 0.1, 0.2, 0.1, 0.1, 0.3, 0.1],  # Temporal
        [0.2, 0.1, 0.7, 0.1, 0.1, 0.2, 0.1, 0.2],  # Cross-modal
        [0.1, 0.2, 0.1, 0.6, 0.1, 0.1, 0.1, 0.3],  # Long-range
        [0.3, 0.1, 0.1, 0.1, 0.5, 0.1, 0.1, 0.1],  # Local
        [0.2, 0.1, 0.2, 0.1, 0.1, 0.4, 0.1, 0.1],  # Global
        [0.1, 0.3, 0.1, 0.1, 0.1, 0.1, 0.6, 0.1],  # Progressive
        [0.1, 0.1, 0.2, 0.3, 0.1, 0.1, 0.1, 0.5]   # Stable
    ])
    
    im4 = ax4.imshow(head_specialization, cmap='Set3', aspect='auto')
    ax4.set_xticks(range(len(heads)))
    ax4.set_yticks(range(8))
    ax4.set_xticklabels(heads, rotation=45)
    ax4.set_yticklabels(['Spatial', 'Temporal', 'Cross-modal', 'Long-range', 
                        'Local', 'Global', 'Progressive', 'Stable'])
    ax4.set_title('Attention Head Specialization', fontsize=12, fontweight='bold')
