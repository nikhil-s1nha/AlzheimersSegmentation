#!/usr/bin/env python3
"""
Neurotoken Extractor - Final Fixed Version
Correctly parses FreeSurfer stats files and extracts neurotokens.
"""
            ...
            for line in f:
                ...
                if line.startswith('#') or not line:
                    ...
                    if 'EstimatedTotalIntraCranialVol' in line:
                        # Parse: # Measure EstimatedTotalIntraCranialVol, eTIV, Estimated Total Intracranial Volume, 2063180.739206, mm^3
                        parts = line.split(',')
                        if len(parts) >= 4:
                            try:
                ...
                # Format: 12  13      1941     1954.5  Left-Pallidum                     94.4060    13.9188     8.0000   122.0000   114.0000
                parts = line.split()
                if len(parts) >= 5:
                    try:
                        ...
                        for i, part in enumerate(parts):
                            if part.replace('.', '').replace('-', '').isalpha():
                                # Extract just the region name part (before any additional numeric data)
                                region_parts = []
    ...
    try:
        ...
        with open(file_path, 'r') as f:
            for line in f:
                ...
                # Parse aparc.stats format: StructName NumVert SurfArea GrayVol ThickAvg ThickStd MeanCurv GausCurv FoldInd CurvInd
                parts = line.split()
                if len(parts) >= 5:
                    region_name = parts[0]
                    try:
                        # Extract the metrics we need
                        surf_area = float(parts[2])  # SurfArea
                        gray_vol = float(parts[3])   # GrayVol
                        thick_avg = float(parts[4])  # ThickAvg (MeanThickness)
                        
                        # Store with consistent naming
                        stats_dict[f"{region_name}_surfarea"] = surf_area
                        stats_dict[f"{region_name}_grayvol"] = gray_vol
                        stats_dict[f"{region_name}_meanthickness"] = thick_avg
...
def extract_aseg_features(stats_dir):
    ...
    if not np.isnan(eTIV) and eTIV > 0:
        for region in SUBCORTICAL_REGIONS:
            if not np.isnan(features[region]):
...
def main():
    ...
    logger.info("Starting neurotoken extraction (final fixed version)...")
    ...
    if all_features:
        ...
        logger.info(f"Sample data (first 3 rows, first 5 columns):")
        logger.info(df.iloc[:3, :5].to_string())
        
        # Show some non-null values
        non_null_cols = df.columns[df.notna().any()].tolist()
        logger.info(f"Columns with data: {len(non_null_cols)}")
        logger.info(f"Sample non-null columns: {non_null_cols[:10]}")
        
        # Show subcortical data specifically
        subcortical_cols = [col for col in df.columns if any(region in col for region in SUBCORTICAL_REGIONS)]
        logger.info(f"Subcortical columns with data: {len(subcortical_cols)}")
        for col in subcortical_cols:
