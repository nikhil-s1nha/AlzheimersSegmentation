...
# Script to check recon-all processing status for OASIS subjects on EC2
# Compares OASIS_Processed directory with subjects directory

# Configuration for EC2
OASIS_PROCESSED_DIR="/home/ubuntu/data/OASIS_Processed"  # Update this to your EC2 data path
SUBJECTS_DIR="/home/ubuntu/subjects"        # Update this to your EC2 subjects path
LOG_FILE="reconall_status_check_ec2.log"
...
echo "🔍 Checking recon-all processing status on EC2..." | tee "$LOG_FILE"
...
if [ ! -d "$OASIS_PROCESSED_DIR" ]; then
    ...
    echo "Please update the OASIS_PROCESSED_DIR variable in this script."
    exit 1
fi

if [ ! -d "$SUBJECTS_DIR" ]; then
    echo -e "${RED}❌ Subjects directory not found: $SUBJECTS_DIR${NC}"
    echo "Please update the SUBJECTS_DIR variable in this script."
    exit 1
fi
...
for subject_dir in "$OASIS_PROCESSED_DIR"/OAS2_*; do
    ...
    for session_dir in "$subject_dir"/session_*; do
        ...
        brain_mgz="$SUBJECTS_DIR/$session_id/mri/brain.mgz"
        aseg_mgz="$SUBJECTS_DIR/$session_id/mri/aseg.mgz"
        aparc_lh="$SUBJECTS_DIR/$session_id/stats/lh.aparc.stats"
        aparc_rh="$SUBJECTS_DIR/$session_id/stats/rh.aparc.stats"
